...
const añadirDescrip = document.getElementById('descripAñadir')
...
document.getElementById("botonAñadir").addEventListener("click", function (event) {
    ...
    let descripAñadir = añadirDescrip.value
    let existenciaAñadir = añadirExistencia.value
    let imagenAñadir = añadirImagen.value

    let van = true

    if (productoAñadir == '' || valorAñadir == '' || existenciaAñadir == '' || imagenAñadir == '' || descripAñadir == '') {
...
window.addEventListener("load", () => {
    ...
    for (let i = 0; i < productos.length; i++) {
        mostraProductos.innerHTML += `<div class="contenedorProductos"><img src="${productos[i].urlImagen}">
        <div class="informacion"><p>${productos[i].nombre}</p>
        <p class="precio"><span>Precio: ${productos[i].valor}</span></p>
        <p class="descrip"><span>${productos[i].descripcion}</span></p>
        <p> Stock: ${productos[i].existencia}</p></div></div>`
    }
})
